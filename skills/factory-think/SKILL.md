---
name: factory-think
description: Use before writing a spec, when a request needs understanding rather than typing - runs every persona lens in parallel over the codebase (product, architect, skeptic, operator, plus builder, auditor and archaeologist when the harness is working on itself), reconciles them, and writes factory/briefs/<slug>.md holding the WHY, the rejected alternatives and the assumptions. Triggers on "/factory-think", "think about", "let's design", "should we build", "figure out what to build", and any feature request vague enough that two people would build different things.
---
...
1. **Orient, then read the rules.** Run `bash scripts/factory_env.sh`. It prints
   the mode, the absolute path to `PRINCIPLES.md`, the persona directories in
   play, and which project-brain files are actually filled in. Never guess at
   `factory-engine/` — in self-development mode there is no such directory, and
   the script is the one place that knows.

   Read the principles it names, then the brain files it lists as filled. Do not
   read the whole codebase yourself — that is what the personas are for, and
   doing their work first biases how you read them.
...
3. **Collect the personas.** List `*.md` in every directory on the `personas:`
   line from step 1. Later directories win on name collision - a project's
   `skeptic.md` replaces the engine's - and everything else joins the set.
   Ignore `README.md`. Print the names that will run.

   In self-development mode that line includes `personas/engine/`, which adds
   **builder**, **auditor** and **archaeologist**: the lenses for changing the
   harness rather than a product. `builder` is the one that matters most there -
   a skill is an instruction for a model, and it is the only reader who can say
   what the instruction actually landed as.
...
5. **Reconcile.** Now you read every persona that ran - four in a project,
   seven when the harness is working on itself. Your job is judgement, not
   stapling:
   ...
   - Where they all agree instantly and thinly, be suspicious. Agents that read
     the same README and repeated it have told you nothing.
...
7. **Write `factory/briefs/<slug>.md`** from `templates/brief.md` under the
   engine path from step 1.
